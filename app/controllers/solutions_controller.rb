class SolutionsController < ApplicationController
  ...
  def create
    ...
    @solution.tags = params[:solution][:tags].split(',').map(&:strip) if params[:solution][:tags].present?

    respond_to do |format|
      if @solution.save
        format.html do
          redirect_to solutions_in_language_path(programming_language: @solution.programming_language),
  ...
  def update
    @solution = @user.solutions.find(params[:id])
    tags_array = solution_params[:tags].split(',').map(&:strip) if solution_params[:tags].present?

    respond_to do |format|
      if @solution.update(tags: tags_array)
        if @solution.update(solution_params.except(:tags))
          format.html do
            redirect_to solutions_in_language_url(programming_language: @solution.programming_language),
  ...
  def destroy
    @solution = @user.solutions.find(params[:id])
    language_of_to_delete = @solution.programming_language
    @solution.destroy!
